...
$ResourceGroup = "rg-dream-interpret-student"
$StorageAccount = "dreaminterpretstd"
$FunctionAppName = "dream-interpret-api-student"
...
```powershell
az functionapp config appsettings set --name dream-interpret-api-student --resource-group rg-dream-interpret-student --settings APP_NAME=dream-interpret APP_VERSION=2026-02-16
```
...
```powershell
az functionapp deployment list-publishing-profiles --name dream-interpret-api-student --resource-group rg-dream-interpret-student --xml > publish-profile.xml
```

If `gh` is installed:

```powershell
gh secret set AZURE_FUNCTIONAPP_NAME --body "dream-interpret-api-student"
...
```bash
curl https://dream-interpret-api-student.azurewebsites.net/api/health
curl https://dream-interpret-api-student.azurewebsites.net/api/version
```
